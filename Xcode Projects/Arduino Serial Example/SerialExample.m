...
- (NSString *) openSerialPort: (NSString *)serialPortFile
                         baud: (speed_t)baudRate
{
	int success;
	...
	if (serialFileDescriptor == -1) { 
		...
		errorMessage = [[NSMutableString alloc] initWithString:@"Error: couldn't open serial port"];
	} else {
		// TIOCEXCL causes blocking of non-root processes on this serial-port
		success = ioctl(serialFileDescriptor, TIOCEXCL);
		if ( success == -1) { 
			errorMessage = [[NSMutableString alloc] initWithString:@"Error: couldn't obtain lock on serial port"];
		} else {
			...
			if ( success == -1) { 
				...
				if ( success == -1) { 
					errorMessage = [[NSMutableString alloc] initWithString:@"Error: couldn't get serial attributes"];
				} else {
					...
					if ( success == -1) {
						errorMessage = [[NSMutableString alloc] initWithString:@"Error: coudln't set serial attributes"];
					} else {
						// Set baud rate (any arbitrary baud rate can be set this way)
						success = ioctl(serialFileDescriptor, IOSSIOSPEED, &baudRate);
						if ( success == -1) { 
							errorMessage = [[NSMutableString alloc] initWithString:@"Error: Baud Rate out of bounds"];
						} else {
							// Set the receive latency (a.k.a. don't wait to buffer data)
							success = ioctl(serialFileDescriptor, IOSSDATALAT, &mics);
							if ( success == -1) { 
								errorMessage = [[NSMutableString alloc] initWithString:@"Error: coudln't set serial latency"];
							}
						}
					}
				}
...
- (IBAction)autoSelectPortButtonPressed:(id)sender
{
    NSString *error;
    
    for (int i = 0; i < [serialListPullDown numberOfItems]; i++)
    {
        error = [self openSerialPort: [serialListPullDown itemTitleAtIndex:i] baud:[baudInputField intValue]];
        
        if (error != nil)
        {
            if (i < [serialListPullDown numberOfItems] - 1)
            {
                [self refreshSerialList:[[[serialListPullDown itemTitleAtIndex:i] stringByAppendingString:@" is not a valid port, returned error: "] stringByAppendingString:error]];
                [self appendToIncomingText:[[[serialListPullDown itemTitleAtIndex:i] stringByAppendingString:@" is not a valid port, returned error: "] stringByAppendingString:error]];
            }
            else
            {
                [self refreshSerialList:[@"No valid port, last port returned error: %@" stringByAppendingString:error]];
                [self appendToIncomingText:[@"No valid port, last port returned error: %@" stringByAppendingString:error]];
            }
        }
