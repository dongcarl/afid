...
{
//	[self.serialCommunicator openSerialPort:[[AFSerialCommunicator availableArduinoSerialPorts] lastObject]];
    AFSerialCommunicator *comm = [[AFSerialCommunicator alloc]init];
    comm.baudRate = B9600;
    [comm closeOpenedSerialPort];
    comm.expectedGestureVectorDimensions = 10;
    [comm openSerialPort:[AFSerialCommunicator availableArduinoSerialPorts].lastObject];
    NSLog(@"next gesture vector: %@",[comm nextGestureVectorFromOpenedSerialPort]);
}
...
{
	[self.recognizer addSingleActionDefinition:[self.serialCommunicator actionDefinitionFromNext:[self.inputNumberTextField intValue] linesForString:[self.actionString stringValue] withBufferSize:0]];
}
...
//        NSLog(@"recognized: %@", [self.recognizer isAction:[self.serialCommunicator getNextIncomingLine]]);
        [self.currentlySeeingText setStringValue:recognized];
//    }
//    NSLog(@"upperBound is %@, lower bound is%@", [self.recognizer.actionDefinitionStack.lastObject upperBound], [self.recognizer.actionDefinitionStack.lastObject lowerBound]);
}
